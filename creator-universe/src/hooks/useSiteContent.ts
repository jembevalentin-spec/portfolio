...
export async function uploadSiteMedia(file: File, kind: "logo" | "owner" | "background-image" | "background-video") {
  const folder = kind === "logo" ? "brand" : kind === "owner" ? "owner" : "hero";
  const bucket = kind === "background-video" || kind === "background-image" ? "jembe-backgrounds" : "jembe-media";
