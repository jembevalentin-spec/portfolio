    ...
    >
      ...
      <div className="relative z-10 max-w-content mx-auto px-6 w-full" style={{ maxWidth: `${site.design.hero.canvasMaxWidth}px` }}>
        <div data-jembe-hero-canvas className="relative min-h-[72svh] lg:min-h-[78svh]">
          <HeroLayer box={site.design.hero.elements.branding} visualId="branding" visualEdit={visualEdit} postVisual={postVisual} className="hidden lg:block">
          ...
          <HeroLayer box={site.design.hero.elements.motto} visualId="motto" visualEdit={visualEdit} postVisual={postVisual} className="hidden lg:block">
            {site.motto && <p className="text-[11px] leading-relaxed text-white/65 max-w-[220px]">{site.motto}</p>}
          </HeroLayer>

          {site.showOwnerPhoto && (
            <HeroLayer box={site.design.hero.elements.owner} visualId="owner" visualEdit={visualEdit} postVisual={postVisual} className="hidden lg:block">
              <motion.div
          ...
          <HeroLayer box={site.design.hero.elements.eyebrow} visualId="eyebrow" visualEdit={visualEdit} postVisual={postVisual}><motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="text-sm text-white/75">{site.heroEyebrow}</motion.p></HeroLayer>

          <HeroLayer box={site.design.hero.elements.headline} visualId="headline" visualEdit={visualEdit} postVisual={postVisual}>
...
function HeroLayer({
  ...
  const startRef = { x: box.x, y: box.y };
  const onPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (!visualEdit || visualDevice() === "mobile" || !visualId) return;
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      postVisual({
        ...
        id: visualId,
        x: Math.max(0, Math.min(100 - box.w, startRef.x + dx)),
        y: Math.max(0, Math.min(100 - box.h, startRef.y + dy)),
      });
    };
    const onUp = () => {
  ...
  return (
    <div
      ...
      style={{ left: `${box.x}%`, top: `${box.y}%`, width: `${box.w}%`, height: `${box.h}%`, opacity: (box.opacity ?? 100) / 100 }}
    >
      {visualEdit && visualId ? <div className="absolute -inset-1 z-50 rounded-lg border border-cyan-300/40 hover:border-cyan-200/90 pointer-events-none" /> : null}
      {children}
    </div>
  );
}
