      ...
      <div className="grid xl:grid-cols-[250px_minmax(0,1fr)_285px] min-h-[calc(100vh-6rem)]">
        ...
        <main className="p-4 md:p-6 overflow-auto bg-black/20">
          ...
          <div className={`mx-auto ${device === "mobile" ? "max-w-[390px]" : "max-w-[1240px]"}`}>
            <div className="relative h-[calc(100vh-190px)] min-h-[680px] max-h-[980px] overflow-hidden rounded-[28px] border border-white/10 bg-[#0a0a0a] shadow-2xl">
              <iframe
                ref={iframeRef}
                title="Jembe full live website editor"
                src={`/?jembeVisualPreview=1&jembeVisualEdit=${mode === "edit" ? "1" : "0"}&device=${device}`}
                onLoad={() => setFrameReady(true)}
                className="absolute inset-0 w-full h-full border-0 bg-black"
                scrolling="yes"
              />
              <div className={`absolute top-3 left-1/2 -translate-x-1/2 pointer-events-none rounded-full bg-black/75 backdrop-blur-md border border-white/10 px-3 py-1.5 text-[9px] uppercase tracking-[.18em] ${mode === "edit" ? "text-cyan-200" : "text-emerald-200"}`}>
                {mode === "edit" ? "Drag highlighted hero elements · scroll the whole site" : "Fully interactive website preview"}
              </div>
            </div>
          </div>
        </main>
        ...
        <aside className="border-l border-stroke p-4 overflow-y-auto">
          ...
          <section className="panel-card mt-4 space-y-4">
            <div>
              <p className="font-display text-lg">Background media</p>
              <p className="text-[11px] text-muted mt-1">Choose a video, photo, or no media for the two large website zones.</p>
            </div>

            <MediaControl
              title="Hero background"
              type={draft.heroBackgroundType}
              onTypeChange={(value) => setDraft((c) => ({ ...c, heroBackgroundType: value }))}
              video={draft.heroBackgroundVideo}
              image={draft.heroBackgroundImage}
              onVideoChange={(value) => setDraft((c) => ({ ...c, heroBackgroundVideo: value }))}
              onImageChange={(value) => setDraft((c) => ({ ...c, heroBackgroundImage: value }))}
              onUpload={async (file, kind) => {
                try {
                  const url = await uploadSiteMedia(file, kind === "video" ? "background-video" : "background-image");
                  setDraft((c) => ({ ...c, ...(kind === "video" ? { heroBackgroundVideo: url, heroBackgroundType: "video" as const } : { heroBackgroundImage: url, heroBackgroundType: "image" as const }) }));
            ...
            <MediaControl
              title="Lower website background"
              type={draft.lowerBackgroundType}
              onTypeChange={(value) => setDraft((c) => ({ ...c, lowerBackgroundType: value }))}
              video={draft.lowerBackgroundVideo}
              image={draft.lowerBackgroundImage}
              onVideoChange={(value) => setDraft((c) => ({ ...c, lowerBackgroundVideo: value }))}
              onImageChange={(value) => setDraft((c) => ({ ...c, lowerBackgroundImage: value }))}
              onUpload={async (file, kind) => {
                try {
                  const url = await uploadSiteMedia(file, kind === "video" ? "lower-video" : "lower-image");
                  setDraft((c) => ({ ...c, ...(kind === "video" ? { lowerBackgroundVideo: url, lowerBackgroundType: "video" as const } : { lowerBackgroundImage: url, lowerBackgroundType: "image" as const }) }));
                  setError(""); setSaved(false);
                } catch (err) { setError(err instanceof Error ? err.message : "Upload failed."); }
              }}
            />

            <NumberField label="Hero overlay" value={draft.heroOverlay} min={0} max={100} step={1} onChange={(v) => setDraft((c) => ({ ...c, heroOverlay: clamp(v, 0, 100) }))} suffix="%" />
            <NumberField label="Lower overlay" value={draft.lowerOverlay} min={0} max={100} step={1} onChange={(v) => setDraft((c) => ({ ...c, lowerOverlay: clamp(v, 0, 100) }))} suffix="%" />
          </section>
...
function MediaControl({
  title, type, onTypeChange, video, image, onVideoChange, onImageChange, onUpload,
}: {
  title: string; type: "video" | "image" | "none"; onTypeChange: (v: "video" | "image" | "none") => void;
  video: string; image: string; onVideoChange: (v: string) => void; onImageChange: (v: string) => void;
  onUpload: (file: File, kind: "video" | "image") => Promise<void>;
}) {
  return (
    <div className="rounded-xl border border-stroke p-3 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs font-medium">{title}</p>
        <select value={type} onChange={(e) => onTypeChange(e.target.value as "video" | "image" | "none")} className="admin-input w-auto">
          <option value="video">Video</option>
          <option value="image">Photo</option>
          <option value="none">None</option>
        </select>
      </div>
      {type === "video" && <input value={video} onChange={(e) => onVideoChange(e.target.value)} className="admin-input" placeholder="Video URL or uploaded file URL" />}
      {type === "image" && <input value={image} onChange={(e) => onImageChange(e.target.value)} className="admin-input" placeholder="Photo URL or uploaded file URL" />}
      <div className="flex gap-2">
        <label className="flex-1 cursor-pointer rounded-lg border border-dashed border-stroke px-3 py-2.5 text-center text-[11px] text-muted hover:text-ink hover:bg-white/[.03]">
          Upload video
          <input type="file" accept="video/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) void onUpload(file, "video"); e.currentTarget.value = ""; }} />
        </label>
        <label className="flex-1 cursor-pointer rounded-lg border border-dashed border-stroke px-3 py-2.5 text-center text-[11px] text-muted hover:text-ink hover:bg-white/[.03]">
          Upload photo
          <input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) void onUpload(file, "image"); e.currentTarget.value = ""; }} />
        </label>
      </div>
    </div>
  );
