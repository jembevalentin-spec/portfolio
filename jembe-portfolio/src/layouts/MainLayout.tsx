...
export default function MainLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const stopNavigationAndSelect = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null;
      const anchor = target?.closest("a[href]");
      if (anchor) event.preventDefault();
      const heroElement = target?.closest("[data-jembe-visual-id]") as HTMLElement | null;
      if (heroElement?.dataset.jembeVisualId) return;
      const section = target?.closest("[data-jembe-section-id]") as HTMLElement | null;
      if (section?.dataset.jembeSectionId && window.parent !== window) {
        window.parent.postMessage({ type: "jembe-visual-select", kind: "section", id: section.dataset.jembeSectionId }, window.location.origin);
      }
    };
    document.addEventListener("click", stopNavigationAndSelect, true);
    return () => document.removeEventListener("click", stopNavigationAndSelect, true);
